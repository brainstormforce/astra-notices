...
class TestNoticeVisibleForCorrectRole extends WP_UnitTestCase {
	...
	public function test_user_can_see_notice() {
		...
		$bsf_admin_notices = new BSF_Admin_Notices();
		...
		$this->assertStringContainsString( 'Notice With Capability', get_echo( array( $bsf_admin_notices, 'show_notices' ) ) );

		// Notice should not be visible to the `subscriber` user.
		wp_set_current_user( $this->subscriber_user_id );
		$this->assertStringNotContainsString( 'Notice With Capability', get_echo( array( $bsf_admin_notices, 'show_notices' ) ) );
	...
	public function test_user_can_see_notice_without_capability() {
		...
		wp_set_current_user( $this->editor_user_id );
		$this->assertStringNotContainsString( 'Notice Without Explicite Capibility', get_echo( array( $bsf_admin_notices, 'show_notices' ) ) );
		...
		// Notice should be visible to the `administrator`.
		wp_set_current_user( $this->administrator_user_id );
		$this->assertStringContainsString( 'Notice Without Explicite Capibility', get_echo( array( $bsf_admin_notices, 'show_notices' ) ) );
	}
}
